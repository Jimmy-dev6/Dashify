...
function normProp(p: SupRow["property"]): { id: string; name: string } | null {
  if (!p) return null;
...
function SupModal({
  ...
  const [priceType, setPriceType] = useState(initial?.price_type ?? "per_stay");
  const [optional, setOptional] = useState(initial?.is_optional !== false);
  ...
  async function submit() {
    ...
    try {
      const url = edit ? `/api/supplements/${initial!.id}` : "/api/supplements";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-800 px-4 py-3">
          <h2 className="text-lg font-semibold text-white">{edit ? "Modifier le supplément" : "Nouveau supplément"}</h2>
        ...
        <div className="space-y-3 p-4 text-sm">
          ...
          <label className="grid gap-1 text-xs font-medium text-gray-400">
            ...
            <textarea
              ...
              rows={3}
              className="rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-white outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
            />
          </label>
          <label className="grid gap-1 text-xs font-medium text-gray-400">
            Logement
          ...
          <label className="flex cursor-pointer items-center gap-2 text-xs text-gray-300">
            <input type="checkbox" checked={optional} onChange={(e) => setOptional(e.target.checked)} className="rounded border-gray-600" />
            Optionnel (sinon inclus au devis)
          </label>
...
export function SupplementsView({ properties }: { properties: PropertyOpt[] }) {
  const [rows, setRows] = useState<SupRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [modal, setModal] = useState<SupRow | null | "new">(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = (await res.json()) as { supplements?: SupRow[]; error?: string };
      if (!res.ok) throw new Error(json.error ?? "Erreur");
      setRows(json.supplements ?? []);
    } catch (e) {
  ...
  return (
    <div>
      <PageHeader
        title="Suppléments"
        description="Options facturables (lit bébé, parking, petit-déjeuner…) sélectionnables sur le devis."
        actions={
          <button
            type="button"
          ...
          <table className="min-w-full divide-y divide-gray-800 text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-800 text-gray-200">
              {loading ? (
                ...
                rows.map((s) => {
                  const prop = normProp(s.property);
                  return (
                    <tr key={s.id} className="hover:bg-gray-800/40">
                      ...
                      <td className="max-w-[12rem] truncate px-4 py-3 text-xs text-gray-400">{s.description || "—"}</td>
                      <td className="px-4 py-3 tabular-nums">{Math.round(s.price).toLocaleString("fr-FR")} XOF</td>
                      <td className="px-4 py-3">{PRICE_LABEL[s.price_type] ?? s.price_type}</td>
